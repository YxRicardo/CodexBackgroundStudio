...
function runtime({ theme, imageDataUrls = {}, imageUrls = {}, artDataUrl, artUrl = imageUrls.hero ?? artDataUrl }) {
  ...
  const ensure = () => {
    ...
    const shellMain = document.querySelector("main[data-app-shell-main-surface]") || document.querySelector("main:has([data-testid='app-shell-header-context-menu-surface'])") || document.querySelector("main.main-surface");
  ...
  const verify = () => {
    ...
    const nativeHome = document.querySelector('[class~="[container-name:home-main-content]"]:has([data-feature="game-source"])') || document.querySelector('[role="main"]:has([data-testid="home-icon"])');
    const markedHome = document.querySelector('.dream-home');
    const missing = [];
    ...
    if (nativeHome && markedHome !== nativeHome) {
      missing.push({ name: "legacy-home-marker", selectors: ['.dream-home'] });
    }
    return {
      id: "codex-theme-v1",
